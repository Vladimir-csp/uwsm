...
class DbusInteractions:
    ...
    def add_systemd_manager(self):
        ...
        self.add_systemd()
        if "systemd_manager" not in self.dbus_objects:
            self.dbus_objects["systemd_manager"] = dbus.Interface(
                self.dbus_objects["systemd"], "org.freedesktop.systemd1.Manager"
            )

    def add_systemd_properties(self):
        "Adds org.freedesktop.systemd1.Manager properties interface"
        self.add_systemd()
        if "systemd_properties" not in self.dbus_objects:
            self.dbus_objects["systemd_properties"] = dbus.Interface(
                self.dbus_objects["systemd"], "org.freedesktop.DBus.Properties"
            )
    ...
    def add_systemd_unit_properties(self, unit_id):
        ...
        unit_path = self.dbus_objects["bus"].get_object(
            ...
            self.dbus_objects["systemd_manager"].GetUnit(unit_id),
        )
    ...
    def reload_systemd(self):
        ...
        self.add_systemd_manager()
        return self.dbus_objects["systemd_manager"].Reload()

    def list_systemd_jobs(self):
        "Lists systemd jobs"
        self.add_systemd_manager()
        return self.dbus_objects["systemd_manager"].ListJobs()
    ...
    def list_units_by_patterns(self, states: list, patterns: list):
        ...
        self.add_systemd_manager()
        return self.dbus_objects["systemd_manager"].ListUnitsByPatterns(
            states, patterns
        )

    def stop_unit(self, unit: str, job_mode: str = "fail"):
        self.add_systemd_manager()
        return self.dbus_objects["systemd_manager"].StopUnit(unit, job_mode)
